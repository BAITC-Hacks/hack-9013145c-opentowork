...
**Панели на крышах:** «Панели на крышах» (`/#/place/roofs`). Для 481 здания
левого берега Астаны — сколько панелей поместится на крыше и сколько энергии они
дадут за год с учётом теней от соседних домов. Это физический расчёт, а не
обученная модель: выработку 1 кВт панелей даёт PVGIS-ERA5, модуль `app/solar/`
добавляет площадь крыши и тени по высотам зданий из OpenStreetMap. Данные лежат
в репозитории, интернет не нужен. У 44% зданий высоты в OSM нет — она принята
по типу здания и помечена на экране. Подробности — [context/02-features.md](context/02-features.md).
...
python -m windcast bids                   # черновики заявок в РФЦ на 01.02–28.02 (DOCX, PDF, CSV)
...
Из прогноза агента собирается черновик **заявки на продажу** по форме приложения 7
Правил организации и функционирования оптового рынка электрической энергии
(приказ Минэнерго РК, V1500010531): отправитель, контрагент — единый закупщик (РФЦ),
операция «Продажа», 24 часовых объёма в МВт с точностью до тысячных.
Готовые файлы на каждые операционные сутки: `artifacts/bids/<дата>/zayavka_<дата>.{pdf,docx,csv,json}`,
через API — `GET /api/v1/bids/2026-02-07/file?format=pdf`.

- **Срок подачи (п. 51):** до 08:00 по Астане суток D−1. В заявку идёт последняя
  опубликованная версия прогноза, выпущенная до срока, — это выпуск 00 UTC (05:00 Астаны),
  а сутки D для него — горизонты 19–43 ч.
- **Корректировки (п. 97–99):** ревизии агента в 06/12/18 UTC превращаются в черновики
  корректировок: час, направление, объём, «было → стало» и крайний срок подачи — не позже
  чем за 2 часа до часа. Корректировка предлагается, если сдвиг ≥ 10% установленной мощности.
- **Справочно** в документе: коридор P10–P90 по каждому часу и риски суток (перепады,
  обледенение, расхождение погодных моделей).
- Все объёмы считает код из прогноза; LLM в заявку не допускается.

Документ помечен «ЧЕРНОВИК»: настоящая заявка подаётся в системе балансирующего рынка
с ЭЦП, интеграции с ней нет. Допущения печатаются в самом документе: номинал турбины
2.5 МВт (в данных не указан, меняется `--rated-mw`), собственное потребление 0,
час «01:00» формы = интервал 00:00–01:00 по Астане — нумерацию стоит уточнить у РФЦ.
...
| `CACHE_CONTEXT_FIELDS` | поля контекста, различающие запросы | `language,region,doc_kind` |
...
| `SEMANTIC_CACHE_SCOPE` | `global` — общий кэш; `user` — ключ включает владельца запроса | `user` |
...
| `WINDCAST_LIVE` | `1` — «Пересчитать» гоняет модель; иначе отдаётся сохранённый прогон агента | `1` в compose |
...
| python-docx | MIT | черновик заявки в DOCX |
| ReportLab | BSD | черновик заявки в PDF |
...
| windpowerlib (oemof) | MIT, текст в `app/wind/UPSTREAM-LICENSE.txt` | кривые мощности турбин для «Новой площадки» |
| Three.js | MIT | 3D-сцены станции и крыш |
| Inter (`@fontsource-variable/inter`) | SIL OFL 1.1 | шрифт интерфейса, встроен в сборку |
| OpenStreetMap | данные ODbL | контуры и этажность зданий (`app/solar/data/`), справочник станций |
| PVGIS-ERA5 (JRC, Еврокомиссия) | свободное использование со ссылкой на источник | месячная радиация и выработка 1 кВт панелей для расчёта крыш |

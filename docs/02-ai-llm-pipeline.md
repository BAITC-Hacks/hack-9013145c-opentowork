...
Вместо прямого вызова `user input → LLM → return` запрос проходит через оркестратор:
кэш, RAG, валидация ответа, запасной провайдер.
...
`MockProvider` возвращает заранее записанные ответы, поэтому проект полностью работает
без интернета и без API-ключей: достаточно `LLM_MODE=mock` (значение по умолчанию).
...
не срабатывает. По умолчанию в режиме `live` стоит 0.92; долю попаданий видно
в `GET /api/v1/ai/stats` и на дашборде Grafana.
...
> после наполнения), поэтому для таблицы, которая растёт с нуля, выбран HNSW.
...
- **Порог релевантности**: если top-1 similarity < 0.5 — считаем, что знаний нет, и явно
  отвечаем «недостаточно данных», а не выдумываем.
...
Пункт 4 — место для правил предметной области (`app/domain/rules.py`): движок проверки
допустимых диапазонов величин в ответе модели готов, но таблица правил `RANGE_RULES`
пока пустая, так что этот слой сейчас ничего не отклоняет.
...
Дашборд Grafana показывает `cache hit rate` и `estimated cost saved` — сколько
запросов и денег снял кэш.

...
не используется: это ещё один контейнер и точка отказа, а на текущих объёмах выигрыша
нет. Переход на неё — возможный следующий шаг при росте данных.
...
> В `DOMAIN_ENTITY.attributes JSONB` хранится всё, что не нужно индексировать:
> JSONB позволяет менять форму данных без новой миграции. Ключевые поля
...
Расширения создаются в двух местах: в `infra/docker/initdb/00-extensions.sql`, который
PostgreSQL выполняет при первом запуске контейнера, и в первой миграции
(`migrations/versions/0001_initial.py`) — на случай внешней базы без initdb.

...
- В K8s — `Secret`; для продакшна — менеджер секретов (External Secrets / Vault).
- Секреты в истории git ищет `gitleaks` в `checks.yml`; вручную —
  `git log -p | grep -iE "api[_-]?key|secret|password"`.
...
`/ready` показывает состояние каждой зависимости — если стек не поднялся, здесь
сразу видно, что именно не работает.

...
### Пустой экран после обновления контейнера

Если console показывает `Failed to fetch dynamically imported module`, вкладка
держит старую сборку, а nginx уже раздаёт новую. Обновить страницу. Для новых
сборок `SceneBoundary` изолирует сбой ленивой 3D-сцены и переключает на 2D.
Ошибка наблюдалась в браузере при параллельной пересборке 23.09.2026.
